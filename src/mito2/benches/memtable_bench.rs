...
fn concurrent_write_rows(c: &mut Criterion) {
    ...
    for num_writers in [8, 16, 32] {
        ...
        group.bench_function(
            format!("partition_tree_same_key_{num_writers}_writers"),
            |b| {
                let codec = Arc::new(SparsePrimaryKeyCodec::new(&metadata)) as Arc<dyn PrimaryKeyCodec>;
        ...
        group.bench_function(
            format!("partition_tree_diff_keys_{num_writers}_writers"),
            |b| {
                let codec = Arc::new(SparsePrimaryKeyCodec::new(&metadata)) as Arc<dyn PrimaryKeyCodec>;
                let memtable = Arc::new(PartitionTreeMemtable::new(
                    1,
